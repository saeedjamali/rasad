...
import { announcementVisibleFilter, ANNOUNCEMENT_SORT } from "@/lib/announcements";
...
function ServiceLamp({ on }) {
  return (
    ...
    >
      <span className={`h-2 w-2 rounded-full ${on ? "bg-emerald-500" : "bg-red-500"}`} />
      {on ? "فعال" : "غیرفعال"}
    </span>
  );
}
...
export default async function Home() {
  ...
  const announcements = await Announcement.find({ isPublic: true, ...announcementVisibleFilter() })
    ...
    <div className="flex-1 bg-[linear-gradient(180deg,#0f3d5f_0%,#0f3d5f_140px,#f4f7fb_140px)]">
      <header className="text-white px-6 py-6 max-w-5xl mx-auto flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-[11px] font-light text-white/55">وزارت آموزش و پرورش</p>
          <p className="text-xs font-light mt-0.5 text-white/70">
            اداره کل آموزش و پرورش خراسان رضوی
          </p>
          <h1 className="mt-3 text-xl md:text-2xl font-semibold">سامانه رصد و پایش درخواست‌ها</h1>
      ...
      <div
        className={`max-w-5xl mx-auto px-6 pb-12 grid gap-6 items-stretch ${
      ...
      >
          ...
          <div className="space-y-4">
            {services.map((s) => (
              <div key={s.id} className="card p-6 self-stretch">
                ...
                {s.description ? (
                  <p className="text-sm leading-7 text-slate-600 whitespace-pre-line">{s.description}</p>
                ) : null}
              </div>
            ))}
          </div>
        ...
        {announcements.length ? (
          <div className="md:col-span-2 space-y-4">
            ...
            {announcements.map((n) => (
              <div key={String(n._id)} className="card p-5 border-r-4 border-r-[#c9a227]">
                <div className="font-semibold">{n.title}</div>
                <AnnouncementDates item={n} />
                <p className="text-sm text-slate-600 mt-2 whitespace-pre-wrap leading-7">{n.body}</p>
                {n.imageUrl ? (
                  <ZoomableImage
                    src={n.imageUrl}
