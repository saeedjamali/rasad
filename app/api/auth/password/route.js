...
export async function POST(req) {
  ...
  if (!settings.allowPasswordLogin) {
    return fail(
      "ورود با رمز عبور از تنظیمات سامانه غیرفعال است. از ورود با پیامک استفاده کنید.",
      403,
    );
  }
  ...
  if (!user.passwordHash) {
    return fail(
      "برای این کاربر رمز عبور تعریف نشده است. از ورود با پیامک استفاده کنید.",
    );
  }
...
export async function PUT(req) {
  ...
  if (confirm && password !== confirm)
    return fail("تکرار رمز عبور مطابقت ندارد");
