...
import { assertAuthSession, createAuthSession } from "./sessions";
...
export function toSessionPayload(user, activeRole, extra = {}) {
  ...
  return {
    ...
    roles,
    activeRole: extra.activeRole ?? activeRole ?? (roles.length === 1 ? roles[0] : null),
    districtCode: String(user.districtCode || ""),
    provinceCode: String(user.provinceCode || ""),
    sid: extra.sid || "",
    epoch: extra.epoch ?? user.sessionEpoch ?? 0,
  };
}

export async function issueUserSession(req, user, activeRole) {
  const rec = await createAuthSession(user, req);
  return toSessionPayload(user, activeRole, { sid: rec.sid, epoch: user.sessionEpoch || 0 });
}

export function refreshSessionPayload(session, user, activeRole) {
  return toSessionPayload(user, activeRole ?? session?.activeRole, {
    sid: session?.sid || "",
...
export async function requireUser(allowedRoles, options = {}) {
  await connectDB();
  ...
  if (!(await assertAuthSession(session, user))) {
    return {
      error: NextResponse.json({ message: "نشست نامعتبر است" }, { status: 401 }),
    };
  }
  ...
  if (!options.skipMaintenance && (await systemBlockedFor(user, role))) {
    return {
      error: NextResponse.json(
