...
function asSettings(doc = {}) {
  return {
    allowNewRequestAfterFinal: Boolean(doc.allowNewRequestAfterFinal),
    allowDistrictAddApplicant: Boolean(doc.allowDistrictAddApplicant),
    allowPasswordLogin: Boolean(doc.allowPasswordLogin),
    smsOnline: doc.smsOnline != null ? Boolean(doc.smsOnline) : process.env.SMS_DEV !== "true",
    systemEnabled: doc.systemEnabled !== false,
    services: Array.isArray(doc.services) ? normalizeServices(doc.services) : normalizeServices(DEFAULT_SERVICES),
  };
}
