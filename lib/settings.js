...
import { REQUEST_EXCEL_ROLES, ROLES, SYSTEM_ACCESS_ROLES } from "./constants";
import { normalizeMaxMainCategories } from "./requestDisplay";
...
export function normalizeSystemEnabled(value) {
  const src = value && typeof value === "object" && !Array.isArray(value) ? value : null;
  const fallback = value !== false;
  return Object.fromEntries(
    SYSTEM_ACCESS_ROLES.map((role) => [role, src ? src[role] !== false : fallback])
  );
}

export function isRoleSystemEnabled(settings, role) {
  return Boolean(role && settings?.systemEnabled?.[role] !== false);
}

export function isSystemOpen(settings) {
  return SYSTEM_ACCESS_ROLES.some((role) => role !== ROLES.admin && isRoleSystemEnabled(settings, role));
}
...
function asSettings(doc) {
  ...
  return {
    ...
    systemEnabled: normalizeSystemEnabled(data.systemEnabled),
    allowRequestSubmit: data.allowRequestSubmit !== false,
    allowMultiMainCategory: Boolean(data.allowMultiMainCategory),
    maxMainCategories: normalizeMaxMainCategories(data.maxMainCategories),
...
export async function saveSettings(patch = {}) {
  ...
  if (patch.systemEnabled != null) {
    next.systemEnabled = normalizeSystemEnabled(patch.systemEnabled);
  }
  if (patch.allowRequestSubmit != null) {
    next.allowRequestSubmit = Boolean(patch.allowRequestSubmit);
  }
  if (patch.allowMultiMainCategory != null) {
    next.allowMultiMainCategory = Boolean(patch.allowMultiMainCategory);
  }
  if (patch.maxMainCategories != null) {
    next.maxMainCategories = normalizeMaxMainCategories(patch.maxMainCategories);
  }
...
export async function systemBlockedFor(user, activeRole) {
  const settings = await getSettings();
  if (userIsAdmin(user)) return false;
  if (activeRole) return !isRoleSystemEnabled(settings, activeRole);
  const roles = user?.roles || [];
  return !roles.some((role) => isRoleSystemEnabled(settings, role));
}

export async function publicStatus() {
  const settings = await getSettings();
  return {
    systemEnabled: isSystemOpen(settings),
    systemEnabledByRole: settings.systemEnabled,
