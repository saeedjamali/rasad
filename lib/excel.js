...
import { APPLICANT_FIELDS, RESULT_LABELS, ROLE_FROM_FA, ROLE_LABELS, statusLabel } from "./constants";
import { requestCategoryLabel } from "./requestDisplay";
...
export function workbookToBuffer(sheets) {
  const wb = XLSX.utils.book_new();
  for (const sheet of sheets || []) {
    const ws = Array.isArray(sheet.rows)
      ? XLSX.utils.json_to_sheet(sheet.rows)
      : XLSX.utils.aoa_to_sheet(sheet.aoa || []);
    if (sheet.cols) ws["!cols"] = sheet.cols;
    XLSX.utils.book_append_sheet(wb, ws, String(sheet.name || "data").slice(0, 31));
  }
...
export function sheetToJsonByName(wb, preferred = []) {
  const names = wb.SheetNames || [];
  const skip = new Set(
    names.filter((n) => /وضعیت|status/i.test(String(n)) && !/نتایج|result|data/i.test(String(n)))
  );
  const wanted = preferred
    .map((n) => names.find((x) => String(x).trim() === n || String(x).includes(n)))
    .find(Boolean);
  const name = wanted || names.find((n) => !skip.has(n)) || names[0];
  if (!name) return [];
  return XLSX.utils.sheet_to_json(wb.Sheets[name], { defval: "", raw: true });
}

export function mapRequestResultRow(row) {
  const g = (...names) => {
    const k = Object.keys(row).find((x) => names.includes(String(x).trim()));
    return k != null ? normalizeCell(row[k]) : "";
  };
  return {
    personnelCode: g("کد پرسنلی", "کد پرسنل", "personnelCode"),
    statusText: g("وضعیت درخواست", "وضعیت", "status"),
    comment: g("توضیحات", "شرح", "توضیح", "comment"),
    destText: g("مقصد نهایی منتقل شده", "مقصد نهایی", "منطقه مقصد نهایی", "resultDest", "dest"),
  };
}
...
export function requestsToRows(list) {
  return list.map((r) => ({
    "کد پیگیری": r.trackingCode || "",
    "عنوان درخواست": r.title || "",
    "توضیحات درخواست": r.description || "",
    "کد پرسنلی": r.personnelCode || "",
    نام: r.firstName || "",
    "نام خانوادگی": r.lastName || "",
    "شماره همراه": r.mobile || "",
    "دسته‌بندی": requestCategoryLabel(r),
    "زیر‌دسته": Array.isArray(r.subcategoryTitles) ? r.subcategoryTitles.join("، ") : "",
    "مقصد پیشنهادی": r.proposedRegionLabel || r.proposedDistrictName || r.proposedDistrictCode || "",
    "منطقه مقصد متقاضی": r.destRegionLabel || r.destDistrict || "",
    "منطقه ارجاع‌شده": r.assignedRegionLabel || r.assignedDistrictName || r.assignedDistrictCode || "",
    وضعیت: statusLabel(r.status, r.result, false),
    "نتیجه نهایی": RESULT_LABELS[r.result] || r.result || "",
    "تاریخ ثبت": formatDateTime(r.createdAt),
    "به‌روزرسانی": formatDateTime(r.updatedAt),
  }));
}
