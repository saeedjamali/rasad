...
export function resolveRegionFromText(map, raw) {
  const text = String(raw || "").trim();
  if (!text) return { region: null, error: null };
  const direct = findRegion(map, text) || findRegionByName(map, text);
  if (direct) return { region: direct, error: null };
  const parts = text.split(/\s*[—–\-]\s*/).map((s) => s.trim()).filter(Boolean);
  if (parts.length >= 2) {
    const combined =
      findRegion(map, parts[0]) ||
      findRegionByName(map, parts.slice(1).join(" — ")) ||
      findRegionByName(map, parts[1]);
    if (combined) return { region: combined, error: null };
  }
  const lowered = text.replace(/\s+/g, " ");
  const hits = [...map.values()].filter((r) => {
    const name = String(r.districtName || "").trim();
    return name === lowered || name.includes(lowered) || lowered.includes(name);
  });
  if (hits.length === 1) return { region: hits[0], error: null };
  if (hits.length > 1) return { region: null, error: `منطقه مقصد «${text}» مبهم است` };
  return { region: null, error: `منطقه مقصد «${text}» یافت نشد` };
}
...
export function decorateRequestItem(item, map) {
  ...
  return {
    ...
    resultDestLabel: (() => {
      const dest = findRegion(map, item.resultDestCode) || findRegionByName(map, item.resultDestName);
      if (dest) return regionLabel(dest);
      return [item.resultDestCode, item.resultDestName].filter(Boolean).join(" — ");
    })(),
  };
}
