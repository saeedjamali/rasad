...
import { categoryIsOffered, mainCategoryCapMessage, normalizeMaxMainCategories } from "@/lib/requestDisplay";
export { categoryIsOffered, requestCategoryIdsOf, requestCategoryLabel } from "@/lib/requestDisplay";
...
export async function resolveRequestCategories(body, { previousIds = [], previousSubIds = [] } = {}) {
  ...
  if (!allowMulti) ids = ids.slice(0, 1);
    ...
    const max = normalizeMaxMainCategories(settings.maxMainCategories);
    if (max && ids.length > max) return { error: mainCategoryCapMessage(max) };
  }
  const prev = new Set((previousIds || []).map(String));
  const prevSubs = new Set((previousSubIds || []).map(String));
  ...
  const blocked = cats.find((c) => !categoryIsOffered(c) && !prev.has(String(c._id)));
  if (blocked) return { error: `دسته «${blocked.title}» در دسترس نیست` };

  const byId = Object.fromEntries(cats.map((c) => [String(c._id), c]));
  const ordered = ids.map((id) => byId[id]).filter(Boolean);
  const parentIds = ordered.map((c) => c._id);
  const rawChildren = await Category.find({ parentId: { $in: parentIds } });
  const allChildren = rawChildren.filter((c) => categoryIsOffered(c) || prevSubs.has(String(c._id)));
