...
export async function visitSummary() {
  ...
  const [visits, visitsTotal, activeIds] = await Promise.all([
    AuthSession.countDocuments({ createdAt: { $gte: today } }),
    AuthSession.countDocuments(),
    AuthSession.distinct("userId", { revokedAt: null, lastSeenAt: { $gte: sinceActive } }),
  ]);
  return { visits, visitsTotal, activeUsers: activeIds.length };
}
