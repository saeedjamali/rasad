...
import { SESSION_ACTIVE_MS } from "./constants";
import { clientIp, publicIpLabel } from "./http";
import { startOfTehranDay } from "./timeSeries";
...
export async function visitSummary() {
  const sinceActive = new Date(Date.now() - SESSION_ACTIVE_MS);
  const today = { createdAt: { $gte: startOfTehranDay() } };
  const [activeIds, auditToday, requestToday, auditTotal, requestTotal] = await Promise.all([
    AuthSession.distinct("userId", { revokedAt: null, lastSeenAt: { $gte: sinceActive } }),
    AuditLog.countDocuments(today),
    RequestLog.countDocuments(today),
    AuditLog.countDocuments(),
    RequestLog.countDocuments(),
  ]);
  return {
    activeUsers: activeIds.length,
    opsToday: auditToday + requestToday,
    opsTotal: auditTotal + requestTotal,
  };
}
