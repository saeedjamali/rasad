import { RESULT_LABELS, STATUSES, STATUS_LABELS } from "./constants";
import { isUserResultNote } from "./requestDisplay";
import { sendSms } from "./sms";

export function finalReviewSmsText(item) {
  return `سامانه رصد: وضعیت درخواست ${item.trackingCode} به «بررسی شده» تغییر کرد.`;
}
...
export function applyAdminRequestStatus(item, { next, result, userId, comment, dest }) {
  ...
  if (next === STATUSES.REVIEW_RESULT) {
    ...
    if (isUserResultNote(note)) item.resultNote = note;
    if (resolved === "approved" && dest?.districtCode) {
      item.resultDestCode = dest.districtCode;
      item.resultDestName = dest.districtName || "";
    }
